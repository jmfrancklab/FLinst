...
class prologix_connection(object):
    ...
    def open(self, ip, port):
        ...
        if versionstring[0:8] == "Prologix":
            ...
            raise RuntimeError(
                "Error! can't find prologix on %s:%d" % (ip, port)
            )
...
class gpib_eth(object):
    ...
    def __init__(self, prologix_instance, address, eos=0):
        ...
        address: int
            the GPIB address
        """
        if prologix_instance is None or not isinstance(
    ...
    def setaddr(self):
        if self.prologix_instance.current_address != self.address:
            self.socket.send(
                ("++addr " + str(self.address) + "\r").encode("utf-8")
            )
    ...
    def tek_query_var(self, varname):
        ...
        temp = temp[
            len(varname) + 2 : -1
        ]  # remove initial space and trailing \n
        if temp[0] == '"':
            return temp[1:-1]
        else:
            return double(temp)
    ...
    def hp_get_curve(self):
        ...
        if points != len(wfrm):
            print(
                "WARNING: Received less points than specified in the preamble"
            )
