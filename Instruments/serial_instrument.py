...
class SerialInstrument(object):
    ...
    def write(self, *args):
        ...
        text = b" ".join(
            j if type(j) is bytes else j.encode("utf-8")
            for j in (str(k) if type(k) is not bytes else k for k in args)
        )
        logger.debug(
            strm(
                "when trying to write, port looks like this:", self.connection
            )
        )
    ...
    def respond(self, *args, **kwargs):
        """Same a write, but also sets the timeout to infinite and
        returns the result.
    ...
    def show_instruments(self):
        """For testing.  Same as :func:`id_instrument`, except that it
        just prints the idn result from all com ports."""
        for j in comports():
    ...
    def demand(self, cmd, value, tries=200, error=1e-2):
        ...
        if type(value) is str:
            ...
            if not m:
                raise RuntimeError(
                    strm(
                        ...
                        ") from",
                        cmd,
                        "but it doesn't match the regular expression'"
                        + value
                        + "'",
                    )
                )
            ...
            if response == 0 and value == 0:
                return response
            if abs((value - response) * 2 / (response + value)) < error:
                self.flush()
                return response
            else:
                raise RuntimeError(
                    strm(
                        ...
                        ") from",
                        cmd,
                        "but it doesn't match the the value of",
                        value,
                    )
                )
    ...
    def check_idn(self, tries=200):
        ...
        while response is None or len(response) == 0 and j < tries:
            ...
            response = self.respond(
                "*IDN?"
            )  # to make sure it's done resetting
        if response is None or len(response) == 0:
            raise ValueError(
                "I tried %d times to contact the %s, to no avail!!!"
                % (tries, self._textidn)
            )
        assert self._textidn in response, (
            repr(response) + " does not match " + repr(self._textidn)
        )
        self.flush(timeout=0.1)
        self.connection.timeout = old_timeout
        return response
    ...
    def id_instrument(self, textidn):
        """A helper function for :func:`init` Identify the instrument that
        returns an ID string containing ``textidn``
        ...
        if len(port_dict) == 0:
            ...
            for j in comports():
                ...
                port_id = (
                    j.device
                )  # based on the previous, this is the port number
                try:
                    with serial.Serial(port_id) as s:
                        s.timeout = 0.1
                        assert s.isOpen(), (
        ...
        for port_id, result in port_dict.items():
            if textidn in result:
                return port_id
        print(
            "I looped through all the com ports and didn't find ",
            textidn,
            "resetting port_dict, and trying again",
        )
