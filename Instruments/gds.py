...
class GDS_Channel_Properties(object):
    """This class controls the channel properties (display (or output), voltage
    level (volts/div), etc.).

    By setting these up as properties, we allow tab completion, and for
    sensible manipulation of parameters.
    """
    ...
    @property
    def voltscal(self):
        """The Channel Scale or volts/div displayed on the GDS for each
        channel.

        Set this by sending a number in scientific notation, such as 1E0 for
        1V/div or 1E-3 for 1mV/div
    ...
    def voltscal(self, vs):
        ...
        logger.debug(
            strm("Setting CH", self.ch, " volt scale to %s volt/div" % vs_str)
        )
        self.gds.write(cmd)
        return
...
class GDS_scope(SerialInstrument):
    ...
    def __init__(self, model="3254"):
        ...
        logger.debug(
            strm(
                "debugging -- identify from within GDS",
                super().respond("*idn?"),
            )
        )
        logger.debug(
            strm("identify from within GDS", super().respond("*idn?"))
        )
    ...
    def timscal(self, ts, pos=None):
        ...
        logger.debug(
            strm("Time scale (sec/div) is set to", self.respond(":TIM:SCAL?"))
        )
        return

    def acquire_mode(self, mode, num_avg=1):
        """ "Set the acquisition mode on the scope. Prints the current mode
        setting.
        ...
        mode : string
            ...
            Possible options:
                ...
                'average' (or 'AVER') = Average mode sampling. Must also set
                     number of averages desired per sample, `num_avg`.

        num_avg : int

            By default set equal to 1. Only needs to be set when in average
            mode sampling. Only
            possible values are powers of 2 up to 2**8:
            2, 4, 8, 16, 64, 128, 256.
        ...
        if num_avg in possible_avg:
            ...
            logger.debug(
                strm("Number of averages set to:", self.respond(":ACQ:AVER?"))
            )
    ...
    def waveform(self, ch=1):
        ...
        * Divides this string at hashtag which separates settings from waveform
        ...
        while int(ready) == 0 and j < 100:
            ...
            j += 1
        if j == 100:
            raise RuntimeError("never became ready!!!")
        ...
        logger.debug("memory is %d" % mem)

        # Generates list of parameters in the preamble
        param = dict(
            [tuple(x.split(",")) for x in preamble if len(x.split(",")) == 2]
        )
        ...
        data_array = np.double(data_array) / np.double(2 ** (2 * 8 - 1))
        ...
        if not all(np.isfinite(x_axis)):
            raise ValueError(
                "your x axis is not finite!! len(data_array) is %s and"
                " 'Sampling Period' is %s"
